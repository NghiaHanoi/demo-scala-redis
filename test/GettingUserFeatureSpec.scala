   ...
   scenario("2.Request getting an user"){      
      ...
      latestIdFuture.onSuccess({
        case s => {
          ...
          When(s"Get latest id success, perform getting user with the latest user id $s.value")
          ...
          userFuture.onSuccess({
            case u =>{
              ...
              Then ("Can convert byte string (Json) to UserEntity instance, name of user should be 'Nguyen Van Nghia'")
              ...
              val jsRet:JsResult[UserEntity] = Json.fromJson[UserEntity](jsonVal)                          
              jsRet.foreach { x => {
                  When ("Got the entity result ")
                  Then(s"Entity name should be 'Nguyen Van Nghia' - entity detail: $x")
