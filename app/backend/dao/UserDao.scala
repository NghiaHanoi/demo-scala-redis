...
class UserDao extends BaseDao[Long, UserEntity]{
  ...
  def all(userIdSeq : Seq[String]):List[Future[Option[String]]] = {
    val userDatFutureList:List[Future[Option[String]]] = List[Future[Option[String]]]()  
    val r = for{
      x1 <- userIdSeq        
    } yield {
  ...
  def create(e:UserEntity):Future[Boolean] ={
    ...
    val userDataString:String = "{\"id\":" + e.id + ",\"name\":\"" + e.name + "\",\"age\":" + e.age + "}"
  ...
  def update(u:UserEntity):Future[Boolean] = {
    ...
    val userDataString = "{\"id\":" + u.id + ",\"name\":\"" + u.name + "\",\"age\":" + u.age + "}"
    tx.watch(uDatKey)
