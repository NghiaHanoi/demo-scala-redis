...
object ClientConnectionActor {
  ...
  object ClientEvent {
    ...
    implicit def clientEventFormat: Format[ClientEvent] = Format(
      ...
      Writes {
        ...
        case other => Json.toJson(
          s"[{'Error':'invalide message $other.toString()'}]")
      })
    //Implicit method required by Application Controller to format WebsocketFrame
    implicit def clientEventFrameFormatter: FrameFormatter[ClientEvent] =
      FrameFormatter.jsonFrame.transform(
        ...
        json => Json.fromJson[ClientEvent](json).fold(
          invalid => throw new RuntimeException(
            "Bad client event on WebSocket: " + invalid),
          valid => valid))
  }
  object UpdateUserEvent {
    ...
    implicit def updateUserFormat: Format[UpdateUserEvent] = (
      ...
      (__ \ "user").format[UserEntity]).apply(
        {
...
class ClientConnectionActor(upstream: ActorRef,
                            userManagerClient: ActorRef) extends Actor {
